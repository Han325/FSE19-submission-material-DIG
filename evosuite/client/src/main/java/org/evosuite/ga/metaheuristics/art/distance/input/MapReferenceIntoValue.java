...
public class MapReferenceIntoValue implements Function<VariableReference, String> {
    ...
    public String apply(VariableReference variableReference) {
        // logger.debug("Variable reference getType: " +
        // variableReference.getType().getTypeName());
        String parameterTypes = Arrays.stream(methodStatement.getAccessibleObject().getParameterTypes())
    ...
    private String extractTypeFromParameter(Statement parameterStmt) {
        // logger.debug("Analyzing parameter statement for type computation: " +
        // parameterStmt.getCode());
        if (parameterStmt instanceof PrimitiveStatement) {
            ...
            if (primitiveValue.isPrimitive()) {
                if (primitiveStatement instanceof IntPrimitiveStatement
                    ...
                    throw new UnsupportedOperationException(
                            this.getClass().getName() + ": found primitive statement " + primitiveStatement + " "
                                    + primitiveStatement.getClass().getName() + ". Not supported yet");
                }
                ...
                if (customEnumDetails.isImplementingCustomInterface()) {
                    throw new UnsupportedOperationException(this.getClass().getName() + ": enum class "
                            + enumPrimitiveStatement + " implements interface " + Properties.CUSTOM_INTERFACE_NAME
                            + ". Not supported yet.");
                }
                ...
                throw new UnsupportedOperationException(
                        this.getClass().getName() + ": found primitive statement " + primitiveStatement + " "
                                + primitiveStatement.getClass().getName() + ". Not a primitive statement.");
            }
            ...
            // or CUSTOM_INTERFACE_NAME for custom classes the implement the
            // CUSTOM_INTERFACE_NAME interface like a Quantity concept for example
            if (customClassDetails.isImplementingCustomInterface()) {
                // logger.debug("Implement custom interface: " +
                // Properties.CUSTOM_INTERFACE_NAME);
            ...
            if (customEnumDetails == null)
                throw new IllegalStateException(
                        "MapReferenceIntoValue extractValueFromParameter: could not find a custom enum from field type name "
                                + simpleTypeName + " of field statement " + fieldStatement.getCode());
            if (customEnumDetails.isImplementingCustomInterface()) {
                throw new UnsupportedOperationException(this.getClass().getName() + ": enum class " + fieldStatement
                        + " implements interface " + Properties.CUSTOM_INTERFACE_NAME + ". Not supported yet.");
            }
            ...
            // We've received a factory method. We must trace back to its parameter
            // to find the true root type.
            MethodStatement methodStmt = (MethodStatement) parameterStmt;
            if (!methodStmt.getParameterReferences().isEmpty()) {
                VariableReference paramOfFactory = methodStmt.getParameterReferences().get(0);
                Statement rootStmt = this.getStatementFromReference(paramOfFactory);
                // Recursively call this same method on the root statement.
                return this.extractTypeFromParameter(rootStmt);
            } else {
                // Factory method with no parameters? The type is unknown.
                // Return a safe default like String.
            ...
            throw new IllegalStateException(this.getClass().getName()
                    + ": statement type must be either PrimitiveStatement, ConstructorStatement or FieldStatement. Type found "
                    + parameterStmt.getClass().getName());
        }
    }

    private String extractValueFromParameter(Statement parameterStmt) {
        // logger.debug("Analyzing parameter statement for value computation: " +
        // parameterStmt.getCode());
        if (parameterStmt instanceof PrimitiveStatement) {
            PrimitiveStatement primitiveStatement = (PrimitiveStatement) parameterStmt;
            ...
            if (primitiveValue.isPrimitive()) {
                 ...
                 * throw new IllegalStateException(this.getClass().getName() + ": enum value " +
                 * enumPrimitiveStatement.getValue() + " is not a string -> " +
                 * enumPrimitiveStatement.getValue().getClass());
                 * }
                 */
                ...
                throw new UnsupportedOperationException(this.getClass().getName() + ": found primitive statement "
                        + primitiveStatement.getValue() + ". Not a primitive value.");
            }
        } else if (parameterStmt instanceof ConstructorStatement) {
            ConstructorStatement constructorStatement = (ConstructorStatement) parameterStmt;
            List<VariableReference> constructorReferences = constructorStatement.getParameterReferences();
            PrintObjectCollection.print(constructorReferences, this.getClass(),
                    "Constructor statement parameter references");
            /*
             * Find the closest reference with respect to the stmt position of this
             * constructor statement.
             * Maybe EvoSuite already handles it (it gives the reference to the last
             * position in which the variable, parameter of
             * this constructor statement, was defined) but I cannot assume it.
             */
            VariableReference closestVariableReference = constructorReferences.stream()
                    .sorted(new Comparator<VariableReference>() {
                        @Override
                        public int compare(VariableReference o1, VariableReference o2) {
                            ...
                            /* Descending ordering */
                            if (stmtPosition1 < stmtPosition2)
                                return 1;
                            else
                                return -1;
                        }
            ...
            try {
                ...
                ex.printStackTrace();
                throw new NumberFormatException(this.getClass().getName()
                        + " parameter of a custom class constructor must be a number! Other formats are not yet supported. ");
            }

            // modify the integer value passed as a parameter to the constructor according
            // to the constructor logic
            String qualifiedClassName = constructorStatement.getDeclaringClassName(); // package_name.class_name
            String simpleClassName = qualifiedClassName.split("\\.")[1]; // class_name

            // PrintObjectCollection.print(ParsedCustomClasses.keys(),this.getClass(),"Key
            // set of parsed custom classes");
            CustomClassDetails customClassDetails = ParsedCustomClasses.getInstance().get(simpleClassName);
            ...
            // We've received a factory method. We must trace back to its parameter
            // to find the true root value.
            MethodStatement methodStmt = (MethodStatement) parameterStmt;
            if (!methodStmt.getParameterReferences().isEmpty()) {
                VariableReference paramOfFactory = methodStmt.getParameterReferences().get(0);
                Statement rootStmt = this.getStatementFromReference(paramOfFactory);
                // Recursively call this same method on the root statement.
                return this.extractValueFromParameter(rootStmt);
            } else {
                // Factory method with no parameters? The value is effectively null/empty.
                return "";
            }
        } else {
            throw new IllegalStateException(this.getClass().getName()
     ...
     * PrimitiveStatement primitiveStatement = (PrimitiveStatement) parameterStmt;
     * logger.debug("Primitive statement value: " + primitiveStatement.getValue());
     * //PrintObjectCollection.print(primitiveStatement.getUniqueVariableReferences(
     * ),this.getClass(),"Primitive statement variable references");
     * PrimitiveValue primitiveValue = new
     * PrimitiveValue(primitiveStatement.getValue());
     * if(primitiveValue.isPrimitive()){
     * String toReturn = "(" + primitiveStatement.getValue() + "[" +
     * primitiveStatement.getReturnType().getTypeName() + "])";
     * return toReturn;
     * }else{
     * throw new
     * IllegalStateException("ExtractValueFromParameter MapReferenceIntoValue: " +
     * primitiveStatement.getValue() + " is not a primitive value");
     * }
     * }else if(parameterStmt instanceof ConstructorStatement){
     * ConstructorStatement constructorStatement = (ConstructorStatement)
     * parameterStmt;
     * List<VariableReference> constructorReferences =
     ...
     * position in which the variable, parameter of
     * //this constructor statement, was defined) but I cannot assume it.
     * VariableReference closestVariableReference =
     * constructorReferences.stream().sorted(new Comparator<VariableReference>() {
     * 
     * @Override
     * public int compare(VariableReference o1, VariableReference o2) {
     * int stmtPosition1 = getStatementPositionFromReference(o1);
     ...
     * }
     * }).findFirst().get();
     * Statement closestParameterStatement =
     * this.getStatementFromReference(closestVariableReference);
     * String parameterValue =
     * this.extractValueFromParameter(closestParameterStatement);
     ...
     * //modify the integer value passed as a parameter to the constructor according
     * to the constructor logic
     * String qualifiedClassName = constructorStatement.getDeclaringClassName();
     * //package_name.class_name
     * String simpleClassName = qualifiedClassName.split("\\.")[1]; //class_name
     * 
     * //PrintObjectCollection.print(ParsedCustomClasses.keys(),this.getClass()
     * ,"Key set of parsed custom classes");
     * CustomClassDetails customClassDetails =
     * ParsedCustomClasses.getInstance().get(simpleClassName);
     * int lowerValue = customClassDetails.getLowerValue();
     * int upperValue = customClassDetails.getUpperValue();
     * 
     * CustomClassLogic customClassLogic = new
     * CustomClassLogic(lowerValue,upperValue);
     * String realParameterValue =
     * customClassLogic.map(parameterValueWithoutParenthesis);
     * 
     * 
     * //ex. Id, Category for categorical numbers (names of custom classes)
     * // or CUSTOM_INTERFACE_NAME for custom classes the implement the
     * CUSTOM_INTERFACE_NAME interface like a Quantity concept for example
     * if(customClassDetails.isImplementingCustomInterface()){
     * logger.debug("Real parameter value: " + realParameterValue + " type: " +
     * Properties.CUSTOM_INTERFACE_NAME);
     ...
     * logger.debug("Parameter type position " + this.loopCounter +
     * " of the method " + this.methodStatement.getMethodName() +
     * " is a custom class " + constructorParameterType);
     * }else{
     * logger.debug("Parameter type position " + this.loopCounter +
     * " of the method " + this.methodStatement.getMethodName()
     * + " is NOT a custom class. It is a " +
     * this.methodParameterTypes.get(this.loopCounter));
     ...
     * }else{
     * throw new IllegalStateException(this.getClass().getName() +
     * " statement type must be either PrimitiveStatement or ConstructorStatement. Type found "
     * + parameterStmt.getClass().getName());
     * }
     * }
     */
    ...
    private String validateEnumParameterType(String parameterType, CustomEnumDetails customEnumDetails) {
        ...
        if (parameterType.equals(realMethodParameterType)) {
            // logger.debug("Parameter type position " + this.loopCounter + " of the method
            // " + this.methodStatement.getMethodName() + " is a custom enum " +
            // parameterType);
            return parameterType;
        } else if (customEnumDetails.getImpementedInterfaces().contains(realMethodParameterType)) {
            // logger.debug("Parameter type position " + this.loopCounter + " of the method
            // " + this.methodStatement.getMethodName() + " is a custom enum " +
            // parameterType + " because it implements real method parameter type " +
            // realMethodParameterType);
            return parameterType;
        } else {
            /*
             * logger.debug("Parameter type position " + this.loopCounter +
             * " of the method " + this.methodStatement.getMethodName()
             * + " is NOT a custom enum. It is a " +
             * this.methodParameterTypes.get(this.loopCounter));
             */
    ...
    private String validateParameterType(String parameterType) {
        if (this.isParameterCorrectType(parameterType)) {
            // logger.debug("Parameter type position " + this.loopCounter + " of the method
            // " + this.methodStatement.getMethodName() + " is a custom class " +
            // parameterType);
            return parameterType;
        } else {
            /*
    ...
    private List<String> getMethodParameterTypes() {
        return Arrays.stream(this.methodStatement.getAccessibleObject().getParameterTypes())
                .map(new Function<Type, String>() {
                    @Override
                    public String apply(Type type) {
                        if (type.getTypeName().contains(".")) {// it is not a primitive value
                            String[] packageStructure = type.getTypeName().split("\\.");
                            return packageStructure[packageStructure.length - 1]; // only stores the simple name
                                                                                  // (without package names, i.e. the
                                                                                  // last element)
                        }
    ...
    private boolean isParameterCorrectType(String constructorParameterType) {
        String realMethodParameterType = this.methodParameterTypes.get(this.loopCounter);
        // logger.debug("isParameterCorrectType. ConstructorParameterType: " +
        // constructorParameterType + "; RealMethodParameterType: " +
        // realMethodParameterType);
        if (constructorParameterType.equals(realMethodParameterType))
            return true;
        return false;
    }
